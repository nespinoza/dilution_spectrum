...
def get_stellar_model(teff=5700.0, logg=4.0, feh=0.0, alpha=0.0, jmag=9.0, stellar_model='ATLAS', workingfolder = ''):
    """Get a stellar spectrum model of arbitary properties with either ATLAS or PHOENIX models
    Parmeters
    ---------
    teff: double
    ...
    workingfolder: str
        The folder where models will be stored. This is optional --- if not defined, the current working directory is defined for this.
    """
    if workingfolder != '':
        if workingfolder[-1] != '/':
                workingfolder += '/'
    ...
    if stellar_model.lower() == 'phoenix':
        w, f = get_phoenix_model(feh, alpha, teff, logg, workingfolder)
    elif stellar_model.lower() == 'atlas':
        w, f = get_atlas_model(feh, teff, logg, workingfolder)

    f = scale_spectrum(w, f, jmag)
    return w, f

def get_phoenix_model(feh, alpha, teff, logg, workingfolder):
    """  
    ...
    stellarmodels_folder_path = workingfolder + 'files/stellarmodels/'
    phoenix_folder_path = workingfolder + 'files/stellarmodels/phoenix/'
    model_folder_path = workingfolder + 'files/stellarmodels/phoenix/' + phoenix_met_and_alpha + '/'
    ...
    if not os.path.exists(phoenix_folder_path + 'wavsol.fits'):
        download('ftp://phoenix.astro.physik.uni-goettingen.de/HiResFITS/WAVE_PHOENIX-ACES-AGSS-COND-2011.fits', phoenix_folder_path + 'wavsol.fits')
...
def get_atlas_model(feh, teff, logg, workingfolder):
    """  
    ...
    stellarmodels_folder_path = workingfolder + 'stellarmodels/'
    atlas_folder_path = workingfolder + 'stellarmodels/atlas/'
    model_folder_path = workingfolder + 'stellarmodels/atlas/' + atlas_met + '/'
...
def get_vega():
    ...
    """
    data = fits.getdata('files/alpha_lyr_stis_009.fits', header=False)
...
def scale_spectrum(w, f, jmag):
    ...
    wT, TT = np.loadtxt('files/jband_transmission.dat', unpack=True, usecols=(0, 1))
